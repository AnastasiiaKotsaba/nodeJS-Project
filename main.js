...
app.post('/posts', async (req, res) => { // async - вказуємо, що функція є асинхронною, щоб можна було використовувати await (await - для розпакування та уникнення промісів)
    try { // Записуємо код в try, щоб у випадку помилки виконати catch (вивести помилку)
		...
		if (!data.name || !data.content || !data.image) {
            // У випадку, якщо поле не заповнене, повертаємо 422 помилку клієнтові та повідомляємо про нестачу даних
        ...
        */
		...
		res.status(201).json(post) // повертаємо клієнтові статус 201 (успішне створення ресурсу) та створений пост
